...
type
  ...
  public
    FShown: Boolean;  // š ’Ç‰Á
    FAfterFreeCount: Integer;
    FPhase: TMemPhase;
    FWorker: TThread;

    procedure AppIdle(Sender: TObject; var Done: Boolean); // š ’Ç‰Á
...
begin
  ...
  Log('Phase: VirtualFree called');

  FPhase := mpAfterFree;
  FAfterFreeCount := 0;

  TimerSample.Interval := 200;   // ‰ð•úŒã‚à“¯‚¶ŽüŠú‚ÅŠÏ‘ª
  TimerSample.Enabled := True;
end;
...
begin
  ...
  if FPhase = mpAfterFree then
  begin
    Inc(FAfterFreeCount);

    Log(
      Format(
        'After Free WS: %d MB (sample %d)',
        [M.WorkingSet div (1024*1024), FAfterFreeCount]
      )
    );

    // š —á‚¦‚Î 10 ‰ñŠÏ‘ª‚µ‚½‚çI—¹
    if FAfterFreeCount >= 10 then
    begin
      TimerSample.Enabled := False;
      FPhase := mpIdle;
      Log('Phase: AfterFree observation finished');
    end;
  end;
end;
